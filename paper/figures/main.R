...
# Figure 3: Volcano plot with SEQC bulk RNA-seq
## Read in DEG list for A vs B contrast
deg_list <- read.table(
    "benchmarks/seqc/data/deg_list.tsv",
    sep = "\t",
    header = TRUE
) |>
    dplyr::mutate(
...
# Categorize genes using pre-computed validation columns
gene_classification <- deg_list |>
    dplyr::group_by(gene_symbol) |>
    dplyr::summarize(
        discovered_by_disize = any(method == "disize" & is_method_deg),
        discovered_by_others = any(method != "disize" & is_method_deg),
        .groups = "drop"
    ) |>
    dplyr::mutate(
...
# Filter data down to the disize subset for plotting and merge classifications
plot_data <- deg_list |>
    dplyr::filter(method == "disize") |>
    dplyr::left_join(gene_classification, by = "gene_symbol") |>
    dplyr::mutate(plot_padj = dplyr::if_else(padj == 0, 1e-320, padj))

# Identify the top 10 unique disize discoveries based on significance to label on the plot
top_unique_labels <- plot_data |>
    dplyr::filter(discovery_status == "Only discovered by disize") |>
    dplyr::slice_min(order_by = plot_padj, n = 10, with_ties = FALSE)

# Construct bulk volcano plot
my_plot <- ggplot(plot_data, aes(x = log2fc, y = -log10(plot_padj), color = discovery_status)) +
...
ggplot2::ggsave(
    filename = "paper/figures/fig-3-bv.png",
    plot = my_plot,
    width = 19,
    height = 16,
    units = "cm"
...
# Figure 4: SFA Validation With Pseudo-bulk ----
benchmark <- read.table(
...
my_plot <- ggplot(plot_data, aes(x = method, y = error, color = method, fill = method)) +
    ...
    base_theme
ggplot2::ggsave(
    filename = "paper/figures/fig-4-rd_sfa.png",
    plot = my_plot,
    width = 20,
    height = 15,
    units = "cm"
...
# Figure 5: DEA Validation With Pseudo-bulk ----
benchmark <- read.table(
    "benchmarks/rnaseq-data/data/scenario-1.tsv",
    sep = "\t",
    header = TRUE
...
combined_plot <- (plot_A / plot_B) +
    ...
    theme(plot.tag = element_text(size = 16, face = "bold"))
ggplot2::ggsave(
    filename = "paper/figures/fig-5-rd_dea.png",
    plot = combined_plot,
    width = 19,
    height = 15,
    units = "cm"
...
# Figure 6: Counting DEGs For The scRNA-seq Case Study ----
...
my_plot <- ggplot(deg_counts, aes(x = method, y = num_of_degs, colour = method, fill = method)) +
    ...
    base_theme
ggplot2::ggsave(
    filename = "paper/figures/fig-6-sc.png",
    plot = my_plot,
    width = 20,
    height = 15,
    units = "cm"
...
# Figure 7: Volcano Plot For The scRNA-seq Case Study ----
...
ggplot2::ggsave(
    filename = "paper/figures/fig-7-sc-vp.png",
    plot = my_plot,
    width = 19,
    height = 16,
    units = "cm"
