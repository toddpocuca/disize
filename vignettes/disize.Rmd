...
vignette: >
  %\VignetteIndexEntry{Introduction to disize}
...
> If the row indices of `counts` and `metadata` do not correspond to the same observation but are named in `counts`, you can specify `obs_name` which will automatically re-order `counts` to match the indices in `metadata`.
...
The authors partitioned *D. melanogaster* cells into two different conditions: observations with `condition = treated` were treated with dsRNAs to knockdown expression via RNAi, while `condition = untreated` were left alone as a control.
Additionally, two sets of duplicates were processed in parallel for each condition with different kits used for library preparation (producing single-read vs paired-end reads).
...
some observations that using the more granular definition may bring some
needed precision.
