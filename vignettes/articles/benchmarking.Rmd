...
simulate_dataset <- function(design_formula, data, n_genes, sparsity, sd) {
    ...
    iodisps <- rlnorm(n_genes, log(10), 1)
    ...
    if (!is.null(design$fixed)) {
        fe_design <- model.matrix(design$fixed, data)[, -1, drop = FALSE]
    } else {
    ...
    list(counts = counts, metadata = data, size_factors = size_factors / sum(size_factors) * length(size_factors))
}
...
get_edger <- function(dataset) {
    ...
    # Extract size factors
    size_factors <- dds$samples$norm.factors
    ...
    benchmark <- sim_pars |>
        dplyr::mutate(d = purrr::pmap(list(n_genes, sparsity, sd), function(n_genes, sparsity, sd) {
            errors <- future.apply::future_replicate(
                ...
                tibble::tibble(
                    disize = sqrt(sum((log(dataset$size_factors) - log(disize_sf))^2)),
                    deseq2 = sqrt(sum((log(dataset$size_factors) - log(deseq2_sf))^2)),
                    edger = sqrt(sum((log(dataset$size_factors) - log(edger_sf))^2))
                )
                },
                simplify = FALSE
            )
        ...
        tidyr::unnest(d) |> 
        dplyr::mutate(
            deseq2_vs_disize = 100 * deseq2 / disize,
            edger_vs_disize = 100 * edger / disize
        ) |>
        tidyr::pivot_longer(
            cols = c(deseq2_vs_disize, edger_vs_disize),
            names_to = "comparison",
            values_to = "relative_precision"
        ) |>
        dplyr::mutate(
            comparison = dplyr::recode(
                comparison,
                "deseq2_vs_disize" = "DESeq2 vs. disize",
                "edger_vs_disize" = "edgeR vs. disize"
            )
        )
...
plot_benchmark <- function(benchmark) {
    the_plot <- ggplot(benchmark, aes(y = relative_precision, x = comparison, color = comparison)) +
        geom_beeswarm() +
        facet_wrap(
            facets = vars(n_genes, sparsity, sd),
            labeller = labeller(
                n_genes = function(x) paste0("# of Genes: ", x), 
                sparsity = function(x) paste0("% Sparsity: ", x),
                sd = function(x) paste0("Nonsparse Standard Deviation: ", x)
            )) +
        labs(x = "Comparison", y = "Relative % Increase in Error") +
        guides(color = "none") +
        theme_classic()
...
sim_pars <- expand.grid(
    "n_genes" = c(10000L, 15000L, 20000L),
    "sparsity" = c(0.10, 0.90),
    "sd" = c(0.50)
) |>
