    ...
    benchmark <- sim_pars |>
        dplyr::mutate(d = purrr::pmap(list(n_genes, sparsity, sd), function(n_genes, sparsity, sd) {
            errors <- future.apply::future_replicate(
                ...
                expr = {
                # Simulate dataset
                dataset <- simulate_dataset(design_formula, metadata, n_genes, sparsity, sd)
...
sim_pars$setting_id <- 1:nrow(sim_pars)
```
...
simulate_dataset <- function(design_formula, metadata, n_genes, sparsity, sd) {
    ...
    list(
        counts = counts, 
        metadata = metadata, 
        size_factors = log(size_factors / sum(size_factors) * length(size_factors)) # Scale for comparisons
    )
}
```
...
```{r, echo = FALSE}
simulate_dataset <- function(design_formula, metadata, size_factors, n_genes, sparsity, sd) {
    # Simulate inverse overdispersion factors
    iodisps <- rlnorm(n_genes, log(10), 0.1)
    ...
    counts <- sapply(1:n_genes, function(i) {
        ...
        # Simulate counts
        rnbinom(nrow(metadata), mu = exp(log_mu), size = iodisps[i])
    })
...
dataset <- simulate_dataset(design_formula, metadata, size_factors, 10000L, 0.5, 1.0)
```
